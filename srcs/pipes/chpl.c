...
/*   Updated: 2025/10/07 14:35:10 by lleichtn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cleanup_child_error(t_pipeline *pipeline, t_cmd *cmd, t_env *env)
{
	t_cmd	*first_cmd;
...
int	child_process(t_child_params *params)
{
	...
	free(params->pids);
	rc = apply_redirections(params->cmd, params->env);
	if (rc != 0)
		cleanup_child_error(params->pipeline, params->cmd, params->env);
	get_first_and_close_hd(params->cmd);
	setup_input(params->cmd, params->pipeline, params->cmd_index);
	setup_output(params->cmd, params->pipeline, params->cmd_index);
	setup_signals_child();
	get_global()->child_pid = 0;
	close_all_pipes(params->pipeline);
	exec_simple_cmd(params->cmd, params->env, rc);
	return (0);
}
